				...
				struct net_device *netdev)
{
	...
	if (fip->mode == FIP_MODE_VN2VN) {
		dev_mc_add(netdev, (u8[]) FIP_ALL_VN2VN_MACS);
		dev_mc_add(netdev, (u8[]) FIP_ALL_P2P_MACS);
	} else
		dev_mc_add(netdev, (u8[]) FIP_ALL_ENODE_MACS);
...
void fcoe_interface_cleanup(struct fcoe_interface *fcoe)
{
	...
	if (fip->mode == FIP_MODE_VN2VN) {
		dev_mc_del(netdev, (u8[]) FIP_ALL_VN2VN_MACS);
		dev_mc_del(netdev, (u8[]) FIP_ALL_P2P_MACS);
	} else
		dev_mc_del(netdev, (u8[]) FIP_ALL_ENODE_MACS);
